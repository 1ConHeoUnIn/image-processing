...
if image is None:
    ...
    # Tăng độ tương phản bằng cách cân bằng histogram
    contrasted_image = cv2.equalizeHist(gray_image)

    # Kiểm tra kích thước
    height, width = contrasted_image.shape
    print("width : ", width)
    print("height : ", height)
    ...
    for i, (x1, y1, x2, y2) in enumerate(crop_coords):
        draw_rectangle(contrasted_image, x1, y1, x2, y2)  # Vẽ hình chữ nhật
        text, cropped_image = extract_text_from_image(contrasted_image, x1, y1, x2, y2)
